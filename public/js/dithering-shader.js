...
class DitheringBackground {
  constructor(canvas, opts = {}) {
    this.canvas = canvas;
    this.width = opts.width || window.innerWidth;
    this.height = opts.height || window.innerHeight;
    this.colorBack = opts.colorBack || '#000000';
    this.colorFront = opts.colorFront || '#D72323';
    this.shape = opts.shape || 'swirl';
    this.type = opts.type || '4x4';
    this.pxSize = opts.pxSize || 4;
  ...
  animate() {
    ...
    const t = (Date.now() - this.startTime) * 0.001 * this.speed;

    gl.clear(gl.COLOR_BUFFER_BIT);
    gl.useProgram(prog);

    if (u.u_time) gl.uniform1f(u.u_time, t);
    if (u.u_resolution) gl.uniform2f(u.u_resolution, this.width, this.height);
    if (u.u_colorBack) gl.uniform4fv(u.u_colorBack, hexToRgba(this.colorBack));
    if (u.u_colorFront) gl.uniform4fv(u.u_colorFront, hexToRgba(this.colorFront));
    if (u.u_shape) gl.uniform1f(u.u_shape, DitheringShapes[this.shape]);
    if (u.u_type) gl.uniform1f(u.u_type, DitheringTypes[this.type]);
    if (u.u_pxSize) gl.uniform1f(u.u_pxSize, this.pxSize);
